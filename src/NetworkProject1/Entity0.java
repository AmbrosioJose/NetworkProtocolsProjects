...
{    
    ...
    public Entity0()
    {
        ...
        for(int i = 0; i < 4; i++){
        	currentShortestPath[i]=distanceTable[0][i];
            //sets all the value of the distance from a node to itself to 0
            distanceTable[i][i] = 0;
            
         }
        //creates a packet and sends packet to 1
        Packet dtPacket = new Packet(0, 1, currentShortestPath);
        NetworkSimulator.toLayer2(dtPacket);
        
        //creates a packet and sends packet to 2
        dtPacket = new Packet(0, 2, currentShortestPath);
        NetworkSimulator.toLayer2(dtPacket);
      
        
        //creates a packet and sends packet to 3
        dtPacket = new Packet(0, 3, currentShortestPath);
        NetworkSimulator.toLayer2(dtPacket);
        
        
        System.out.println("Entity0 initialized");
        printDT();
    }

...
{    
    ...
    public Entity2()
    {
        ...
        for(int i = 0; i < 4; i++){
        	currentShortestPath[i]=distanceTable[2][i];
            //sets all the value of the distance from a node to itself to 0
            distanceTable[i][i] = 0;
            
         }
        //creates a packet and sends packet to 0
        Packet dtPacket = new Packet(2, 0, currentShortestPath);
        NetworkSimulator.toLayer2(dtPacket);
        
        //creates a packet and sends packet to 1
        dtPacket = new Packet(2, 1, currentShortestPath);
        NetworkSimulator.toLayer2(dtPacket);
        
        //creates a packet and sends packet to 3
        dtPacket = new Packet(2, 3, currentShortestPath);
        NetworkSimulator.toLayer2(dtPacket);
        
        System.out.println("Entity2 initialized");
        printDT();
    }
